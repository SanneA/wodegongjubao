...
*/
        ...
        if ($this->db->ConType()<=3) //ms sql
        {
            $q = $this->db->query("SELECT TOP $cont autothor, convert(date, indate,120) as indate, ntitle,ntype,news FROM mwc_news ORDER BY nid DESC");
        ...
        {
            $q = $this->db->query("SELECT * FROM mwc_news ORDER by nid desc LIMIT $cont");
        }

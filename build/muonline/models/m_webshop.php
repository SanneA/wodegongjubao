...
class m_webshop extends MuonlineUser
{
    ...
    {
        $r = $this->db->query("SELECT CONVERT(VARCHAR(3840), Items, 2) as Items FROM warehouse WHERE AccountID='test'")->FetchRow();
        $inventory = strtoupper($r["Items"]);
    ...
    function adding($id,$price,$leng)
    {
        ...
        if($puting["statez"] == 0)//если все хорошо
        {
            ...
            $this->db->query("INSERT INTO mwc_web_shop ([col_itemID],[col_idemGroup],[col_Name],[col_hex],[col_serial],[col_serial2],[col_level],[col_isOpt],[col_isExc],[col_isAnc],[col_isSock],[col_isSkill],[col_isPVP],[col_isHarmony],[col_eq],[col_prise],[col_priseType],[col_isMy]) VALUES ({$wegot["id"]},{$wegot["group"]},'{$wegot["name"]}','{$wegot["hex"]}','{$wegot["serial1"]}','{$wegot["serial2"]}',{$wegot["level"]},{$wegot["intopt"]},{$wegot["excnum"]},{$wegot["ancnum"]},{$wegot["sockHex"]},{$wegot["isSkill"]},{$wegot["ispvp"]},{$wegot["harmonyOpt"]},'{$wegot["equipmentn"]}',{$price},1,0)");
